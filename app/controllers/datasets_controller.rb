class DatasetsController < ApplicationController
  ...
  def show
    ...
    @ingest_health_summary = ingest_health_summary
  end
  ...
  def ingest_health_summary
    ingest_attempt = @latest_delivery_attempts["ingest"]
    return nil unless ingest_attempt

    stale_minutes = ingest_health_stale_minutes
    orphan_lookback_minutes = ingest_health_orphan_lookback_minutes
    orphan_threshold = ingest_health_orphan_alert_threshold

    alerts = []
    freshness_message = nil
    health_state = "ok"

    if ingest_attempt.response_received_at.present?
      age_minutes = ((Time.current - ingest_attempt.response_received_at) / 60).floor
      ...
      age_minutes = ((Time.current - ingest_attempt.created_at) / 60).floor
    ...
    if ingest_attempt.response_failed?
      health_state = "critical"
      alerts << "Latest Medusa response reported failure."
    end

    orphan_count = recent_orphaned_responses_for_dataset(lookback_minutes: orphan_lookback_minutes)
  ...
  def recent_orphaned_responses_for_dataset(lookback_minutes:)
    lower_bound = lookback_minutes.minutes.ago
    base = IngestResponseEvent.orphaned.where(received_at: lower_bound..Time.current)
