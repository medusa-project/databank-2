...
RSpec.describe Datafile::Viewable do
  describe ".peek_type_from_mime" do
    it "returns all_text for text files under the display threshold" do
      expect(Datafile.peek_type_from_mime("text/plain", Datafile::Viewable::ALLOWED_DISPLAY_BYTES)).to eq("all_text")
    end

    it "returns part_text for text files above the display threshold" do
      expect(Datafile.peek_type_from_mime("text/plain", Datafile::Viewable::ALLOWED_DISPLAY_BYTES + 1)).to eq("part_text")
    end

    it "returns archive for recognized archive MIME subtypes" do
      expect(Datafile.peek_type_from_mime("application/zip", 1024)).to eq("archive")
    end

    it "returns none for unknown MIME types" do
      expect(Datafile.peek_type_from_mime("application/octet-stream", 1024)).to eq("none")
    end

    it "returns markdown for markdown MIME types" do
      expect(Datafile.peek_type_from_mime("text/markdown", 1024)).to eq("markdown")
    end
  end
  ...
  describe "text predicates" do
    ...
    it "treats all_text and part_text as text previews" do
      all_text = build(:datafile, dataset: dataset, attach_binary: false, peek_type: "all_text")
      part_text = build(:datafile, dataset: dataset, attach_binary: false, peek_type: "part_text")
      archive = build(:datafile, dataset: dataset, attach_binary: false, peek_type: "archive")
  ...
  describe "peek persistence on save" do
    ...
    it "stores all_text peek content for small text uploads" do
      ...
      expect(datafile.reload.peek_type).to eq("all_text")
    ...
    it "stores part_text peek content for large text uploads" do
      ...
      datafile.reload
      expect(datafile.peek_type).to eq("part_text")
    ...
    it "does not overwrite existing peek_content" do
      datafile = create(:datafile, dataset: dataset, peek_type: "all_text", peek_content: "precomputed")
    ...
    it "stores markdown peek type and content based on filename extension" do
      ...
      datafile.reload
      expect(datafile.peek_type).to eq("markdown")
