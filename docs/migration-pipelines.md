...
## 3) Secure bundle import (legacy export to databank-2)
...
If `CHECKSUM` or `MANIFEST` is provided, the importer verifies SHA256 integrity before writing any records. If `MANIFEST` includes `record_count`, that value is also validated against processed NDJSON rows.

## 4) Production export to secure copy to local import
