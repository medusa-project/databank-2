...
Create `~/shared/config/credentials/demo.key` on the demo host with the correct
credentials key. Ensure `~/shared/nginx.conf.erb` exists if your host setup
requires it.
...
After demo is stable, repeat the same process for production using `production`
stage commands.
