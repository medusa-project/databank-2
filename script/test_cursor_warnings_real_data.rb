...
test_dir = Rails.root.join('tmp', 'test_real_bundle')
...
puts "\nStarting import test with warning capture..."
puts "=" * 80
...
begin
  ...
  if defined?(CursorPaginationWarningCapture)
    ...
    puts "=" * 80
    if warnings.any?
      puts "⚠ WARNINGS FOUND! First 3 warnings:"
      warnings.first(3).each_with_index do |warning, idx|
        puts "\n--- Warning #{idx + 1} ---"
